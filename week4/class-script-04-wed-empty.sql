...
SELECT Site, Avg(Snow_cover) FROM Snow_cover
    GROUP BY Site
    ORDER BY MAX(Snow_cover) DESC
    LIMIT 3;

    --can also have it as ORDER BY -Snow_cover
...
SELECT Site, Avg(Snow_cover) AS avg_cover FROM Site_avg_snowcover
    WHERE Snow_cover > 0
    GROUP BY Site;
